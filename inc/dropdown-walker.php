  ...
  private $toggle = true;
    ...
    //$item_html = str_replace('<li', '<li role="menuitem"', $item_html);
		if ( $item->is_dropdown && ( 1 != $args->depth ) ) {
			...
			$item_html = str_replace( '<a', '<a class="dropdown-toggle" role="menuitem" data-toggle="dropdown" tabindex="0"', $item_html );
			$item_html = str_replace( '</a>', '<b class="caret"></b></a>', $item_html );
    } else if ( $item->menu_item_parent == 0) {
			$item_html = str_replace( '<a', '<a class="dropdown-toggle" role="sub-menu" tabindex="0"', $item_html );
    } else {
