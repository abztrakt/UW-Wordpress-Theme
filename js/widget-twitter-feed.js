$(document).ready(function() {
  $('.twitter-feed').each(function() {
    ...
    $.getJSON('https://api.twitter.com/1/statuses/user_timeline.json?callback=?', data, function(json) {
      json.reverse();
      while(json.length) {
        var tweet     = json.pop()
          , hasAuthor = tweet.entities.user_mentions.length > 0
          , hasUrl    = tweet.entities.urls.length > 0
          , img       = hasAuthor ? 'https://api.twitter.com/1/users/profile_image?size=mini&screen_name=' + tweet.entities.user_mentions[0].screen_name :
